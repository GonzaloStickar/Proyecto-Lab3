            ...
            if (a.getEstado() != (EstadoAsignatura.APROBADA)) {
                for (String correlativa : a.getMateria().getCorrelatividades()) {
                    ...
                    if (asignaturaCorrelativa != null) {
                        if (!asignaturaCorrelativa.getMateria().getCorrelatividades().isEmpty()) {for (String correlativaDeAsignaturaCorrelativa : asignaturaCorrelativa.getMateria().getCorrelatividades()) {Asignatura asignaturaCorrelativaDeAsignaturaCorrelativa = buscarAsignaturaByNameAsignaturasAlumno(alumno, correlativaDeAsignaturaCorrelativa);if (asignaturaCorrelativaDeAsignaturaCorrelativa!=null) {if (asignaturaCorrelativaDeAsignaturaCorrelativa.getEstado()!= EstadoAsignatura.APROBADA) {throw new AlumnoServiceException("La asignatura "+asignaturaCorrelativaDeAsignaturaCorrelativa.getNombreAsignatura()+" debe estar aprobada", HttpStatus.OK);}}}}
